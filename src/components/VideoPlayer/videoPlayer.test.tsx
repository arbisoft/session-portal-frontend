...
jest.mock('@mui/icons-material/PlayCircle', () => {  // Mock Material-UI PlayCircle icon
  const MockPlayCircleIcon = (props: any) => {
...
jest.mock('@vidstack/react', () => {
  ...
  return {
    MediaPlayer: React.forwardRef<any, any>(({ 
      children, 
      className, 
      src, 
      title, 
      ...
      React.useImperativeHandle(ref, () => mockMediaPlayerInstance);
      ...
      return (
        <div 
          ...
          data-src={src}
          data-title={title}
          crossOrigin={crossOrigin?.toString()}
...
jest.mock('@vidstack/react/player/layouts/default', () => ({
  DefaultVideoLayout: ({ icons }: { icons: any }) => (
...
describe('VideoPlayer', () => {
  ...
  const { 
    ...
    __mockUseMediaState: mockUseMediaState,
    __mockMediaPlayerInstance: mockMediaPlayerInstance
  } = jest.mocked(require('@vidstack/react'));
  ...
  describe('Props Handling', () => {
    it('applies crossOrigin prop correctly', () => {
      render(<VideoPlayer {...defaultProps} crossOrigin={false} />);
      
      const mediaPlayer = screen.getByTestId('media-player');
      expect(mediaPlayer).toHaveAttribute('crossOrigin', 'false');
    });
  ...
  describe('Component Lifecycle', () => {
    ...
    it('updates subscription when onVideoEnded prop changes', () => {
      ...
      const { rerender } = render(
        <VideoPlayer {...defaultProps} onVideoEnded={onVideoEnded1} />
      );
      
      expect(mockSubscribe).toHaveBeenCalledTimes(1);
      
      rerender(
        <VideoPlayer {...defaultProps} onVideoEnded={onVideoEnded2} />
      );
